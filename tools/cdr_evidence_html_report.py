...
HTML_SCHEMA = "ecss-cdr-evidence-html-report-v4"
...
<div class="filters"><input id="evidence-search" type="search" placeholder="Search label, caller ref or classification" aria-label="Search evidence"><select id="evidence-classification" aria-label="Filter by classification"><option value="">All classifications</option>{classification_options}</select><button id="evidence-reset" type="button">Reset</button><button id="evidence-export" type="button">Export visible CSV</button><button id="evidence-print" type="button">Print visible</button></div>
...
(() => {{
  ...
  const apply = () => {{
    ...
    printButton.disabled = visible === 0;
  }};
  const csvCell = (value) => {{
  ...
  printButton.addEventListener('click', printVisible);
  apply();
}})();
</script>
</body></html>"""
