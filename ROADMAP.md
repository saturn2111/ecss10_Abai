...
- [x] r56 local `Export visible Markdown` handoff for the exact currently visible subset, including active filter context and raw-evidence disclaimer; exact SHA `b00aadfa8059a69a1374c4bfd07cefa72d689ef6` passed Forgejo run 700 and is merged.
...
- Verified portable operator handoff now includes HTML/TSV/CSV/JSON/Markdown/print-to-PDF plus session-only bookmarks and visible-subset filtering.
...
- Semantic correlation is now deliberately waiting for real sanitized queue CDR evidence instead of inventing `Duration`/queue-wait meaning.
