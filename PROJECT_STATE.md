...
- добавлен `selected_operator_timing_state`: он детерминированно описывает полноту `T_ECD`/`T_DBA` только на уже выбранной exact operator record (`complete_timing`, missing-поля или отсутствие selection), не выбирая другую запись и не создавая новых live-выводов;
